...
| Area | Details |
|------|---------|
| **Boot** | Real-mode → protected-mode transition, A20 gate enable, GDT loading, and proper high-memory stack allocation. |
| **Disk I/O** | INT 0x13 with CHS addressing, multi-track floppy drive reads. |
| **Kernel loading** | Safe overlapping copy via backwards `rep movsd` (source `0x0600` → dest `0x2000`). |
| **VGA text mode** | 80×25 text buffer, hardware cursor tracking, terminal scrolling, hex rendering, custom overflow-proof integer printing, and `\t` tab stops. |
| **PS/2 keyboard** | Interrupt-driven ring buffer, shift/caps lock parsing, command history cycle via arrow keys. |
| **Interrupts** | IDT with 32 ISRs (exceptions) and 16 IRQs. Safe boot order implementation (LIDT → PIC remap → STI) to prevent spurious triple faults. |
| **Timer (PIT)** | Thread-safe atomic tick increment via inline assembly `LOCK XADD` and stable overflow-proof wrappers for time delay functions. |
| **Serial I/O** | COM1 serial port driver ready for kernel logging, telemetry, and low-level debugging. |
...
| **GCC** (i686, `-m32`) | Compiles kernel C and assembly to 32-bit x86 code |
...
│    • Sets up real-mode segments and 16-bit stack at     │
│      0x7C00 (grows downward safely into free space)     │
...
│    • Relocates 32-bit stack pointer ESP to 0x90000      │
│      (safely clear of any kernel code/data regions)     │
│    • Backwards rep movsd: copies kernel from 0x0600     │
│      to 0x2000 (count = KERNEL_SECTORS * 128 dwords)    │
│    • Calls KERNEL_OFFSET (0x2000) → _start (entry.S)    │
├─────────────────────────────────────────────────────────┤
│  _start() -> kernel_main() (Assembly & C integration)   │
│    • entry.S resolves underscore targets and passes     │
│      control directly to kernel_main()                  │
│    • init_gdt()   — reloads GDT with user-mode entries  │
│    • init_idt()   — loads IDT first, remaps PIC layout, │
│                     then triggers STI to allow IRQs     │
│    • init_timer() — initializes PIT and atomic engine   │
...
| `0x7C00` | 512 B | Bootloader MBR and 16-bit stack top (`SP = 0x7C00`) |
| `0x7E00`–`0x8FFF` | ~4.5 KB | Bootloader variables/data space |
| `0x90000` | variable | 32-bit Protected Mode Stack (`ESP = 0x90000`, grows downward) |
...
.
├── boot/
│   └── bootloader.asm        # MBR bootloader (real-mode, stack 0x7C00 → PMode trampoline, stack 0x90000)
├── kernel/
│   ├── entry.S               # Kernel entry point (_start → kernel_main, resolved underscore issues)
│   ├── kernel.c              # Shell loop, command parsing, readline with history
│   ├── cpu/
│   │   ├── gdt.c             # GDT entries (null, code, data, user code, user data)
│   │   ├── gdt.h
│   │   ├── idt.c             # IDT entry mapping, safe initialization ordering to fix triple faults
│   │   ├── idt.h             # registers_t struct, interrupt_handler_t typedef
│   │   ├── interrupt.S       # ISR and IRQ stubs (GAS macros), common assembly handler
...
│       ├── screen.c          # VGA text mode: print_char, overflow-safe print_int, backspace handler
│       ├── screen.h          # Character utilities and function prototypes
│       ├── serial.c          # COM1 serial port driver for kernel logging/debugging
│       └── serial.h
...
* **Safe 32-bit Stack Placement** — High-precision execution environments require reliable stacks. The 32-bit PMode stack pointer is explicitly assigned to `0x90000` inside the trampoline, keeping it entirely detached from executable boundaries.
* **Strict Interrupt Pipeline** — Setting up low-level interrupts without an active IDT reference creates immediate edge crashes. The kernel loads the gate description structural boundaries using `lidt` before opening up ports to PIC device interaction.
* **Atomic Time Management** — Peripheral state clocks are vulnerable to context race-conditions. System time ticks utilize the raw inline instruction `lock xaddl` to freeze bus arbitration and manage hardware clock structures cleanly.
...
| `0x3F8` | Write/Read | COM1 Serial Port Data base (transmits telemetry data bytes) |
